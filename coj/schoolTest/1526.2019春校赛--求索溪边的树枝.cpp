 ...
 *思路：暴力+剪枝
 ...
 2. 对n根树枝可以先从大到小排序，如果第一个的树枝比较大，那么dfs搜索的深度可以减小，因为只要判断拼出的树枝大于需要的长度即可返回false
 3. 有n根树枝, 那么拼成的m根树枝必定满足 m <= n(不可能树枝个数越拼越多), 所以该m可以从n开始遍历，直到1, n根树枝的情况就是所有树枝长度都相同，这里可以在从大到小排序后判断最小的一根树枝和大的一根树枝是否相同，如果相同，那么直接输出
 4. 对于每一次搜索得到的树枝长度，如果该长度+剩余的长度 < 需要的长度,那么可以直接返回false
 5. 如果当前长度+所有树枝中最小的长度 > 需要的长度, 不管这个最小长度有没有被选, 都可以直接返回false
 6. 如果选择的某个长度不能满足，而后面重复出现该长度，则直接跳过, 比如4 2 2 2 1, 如果搜索到第一个2不满足最终的和为需要的长度, 那么可以跳过后面连续的2, 直接找1 
