...
我们搬砖时都是将砖叠成一摞再搬。小胖的力气非常大，当然希望尽可能的摞到最高,
但是只有上面砖的底面两条边都小于下面砖底面相对的两条边时， 才能叠稳，注意砖可以旋转。
(如: 长宽高为[3,4,5]的砖能叠在[2，5，5]，[5,4,3]上,不能叠在[2,4,3]上)。 帮小胖算算他第一次最多能搬走多少砖。
输入描述:
...
/*
 思路：由于每一种砖不限个数，那么对于同一种砖，就只有三种情况，而由于后面一个砖要放上去，必须满足长宽小于前面一个砖，
 所以同一种砖不能放超过2个，因为假如有砖a, b, c(a < b < c), 则ab bc ac,最多可以用ab bc两个砖, 
 因此在输入的时候先对砖的长宽高排序，再构造该砖的三种情况，30种砖就转化为90个砖，
 对该90个砖排序，就可以使用最长上升子序列来做
 *
 */
